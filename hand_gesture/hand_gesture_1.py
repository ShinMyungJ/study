...
train = pd.read_csv("D:/_data/hand_gesture/train.csv")
test = pd.read_csv("D:/_data/hand_gesture/test.csv")
submission = pd.read_csv("D:/_data/hand_gesture/sample_submission.csv")
...
def create_model(num_layer, mid_units, num_filters):
    ...
    model.add(Dense(mid_units, activation='gelu'))
...
def cnn_objective(trial: Trial) -> float:
    ...
    for n, (train_idx, val_idx) in enumerate(kfold.split(train_x, train.target)):
        ...
        optimizer = AdaBeliefOptimizer(learning_rate=trial.suggest_uniform("learning_rate", 0.0005, 0.005))
...
for i, (train_idx, val_idx) in enumerate(skf.split(train_x, train.target)):
    ...
    # 모델 Complie
    optimizer = AdaBeliefOptimizer(learning_rate=cnn_study.best_params['learning_rate'])
