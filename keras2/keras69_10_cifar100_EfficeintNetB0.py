...
# [실습] cifar100을 넣어서 완성할 것

# vgg trainable : True, False
# Flatten / GAP
# 위 4가지 결과와, 지금까지 본인이 cifar100을 돌려서 최고치가 나온거와 비교!!!
# time도 명시해라!!!
...
net = EfficientNetB0(weights='imagenet', include_top=False,
              input_shape=(32,32,3))
...
model_path = "".join([filepath, 'k69_10_', datetime, '_', filename])
es = EarlyStopping(monitor='val_loss', patience=15, mode='auto', verbose=1, restore_best_weights=True)
# mcp = ModelCheckpoint(monitor="val_loss", mode="auto", verbose=1, save_best_only=True, filepath=model_path)
lr = ReduceLROnPlateau(monitor='val_loss', patience=5, mode='min', verbose=1, factor=0.5)

start = time.time()

hist = model.fit(x_train, y_train, epochs=100, batch_size=32, validation_split=0.25, callbacks=[es, lr])#, mcp])
